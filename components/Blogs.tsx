...
const Blogs: React.FC<BlogsProps> = ({ blogs }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-8 md:py-12">
        ...
        {blogs.map((blog) => (
            <div key={blog.slug} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col hover:shadow-xl transition-all animate-reveal">
            ...
            <div className="p-4 md:p-6 flex-1 flex flex-col">
                ...
                <button onClick={() => navigate(`/blogs/${blog.slug}`)} className="mt-auto bg-univet-blue text-white py-2 md:py-3 rounded-lg font-black text-[10px] uppercase tracking-widest hover:bg-univet-gold hover:text-univet-blue transition-all">
                Read Analysis
                </button>
            </div>
            </div>
